...
export default class Chain extends IChain {
    ...
    async deployEarningPools(config: Config): Promise<{ [key: string]: Contract}> {
        return {
            'WETH': await this.deployAaveEarningPool(config, config.wethTokenAddress),
            'LINK': await this.deployAaveEarningPool(config, config.linkTokenAddress),
            'DAI': await this.deployAaveEarningPool(config, config.daiTokenAddress),
            // 'LINK': await this.deployCompoundV2EarningPool(config, config.daiTokenAddress),
        }
    }
